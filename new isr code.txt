...
uint16_t get_touch(uint8_t touch_type)
{
    uint16_t data_rec[2];
    uint16_t touch;
    uint8_t reg_1;
    ...
    if(touch_type == 0)
    {
        reg_1 = Read_Register_X_1;
        reg_2 = Read_Register_X_2;
    }
    else if(touch_type == 1)
    {
        reg_1 = Read_Register_Y_1;
        reg_2 = Read_Register_Y_2;
    }
    ...
    data_rec[0] = I2C_MasterReadByte(I2C_NAK_DATA);
    
    I2C_MasterSendStop();
    
    I2C_MasterSendStart(FT6206_ADDR, WRITE);
    
    I2C_MasterWriteByte(reg_2);
    
    I2C_MasterSendRestart(FT6206_ADDR, READ);
    
    data_rec[1] = I2C_MasterReadByte(I2C_NAK_DATA);
    
    I2C_MasterSendStop();
    
    I2C_Stop();
    
    touch = ((data_rec[0] << 8) & 0x0F00) | (data_rec[1] & 0x00FF);
